...
def test_mass_properties_invalid_mass():
    ...
    with pytest.raises(ValueError, match="Mass must be positive."):
        MassProperties(mass_slug=0.0, inertia_body_slug_ft2=inertia, cg_body_ft=cg)
        
    # Test negative mass
    with pytest.raises(ValueError, match="Mass must be positive."):
        MassProperties(mass_slug=-10.0, inertia_body_slug_ft2=inertia, cg_body_ft=cg)
...
def test_mass_properties_invalid_inertia():
    ...
    with pytest.raises(ValueError, match=r"Inertia Matrix must have shape \(3, 3\)."):
        MassProperties(mass_slug=10.0, inertia_body_slug_ft2=invalid_inertia, cg_body_ft=cg)
...
def test_mass_properties_invalid_cg():
    ...
    with pytest.raises(ValueError, match=r"Center of Gravity must have shaper \(3,\)."):
        MassProperties(mass_slug=10.0, inertia_body_slug_ft2=inertia, cg_body_ft=invalid_cg)